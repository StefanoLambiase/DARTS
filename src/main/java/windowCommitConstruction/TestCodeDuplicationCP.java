...
public class TestCodeDuplicationCP extends JPanel implements ListSelectionListener {
    private ArrayList<TestCodeDuplicationInfo> classesWithTestCodeDuplication;
    ...
    public TestCodeDuplicationCP(ArrayList<TestCodeDuplicationInfo> classesWithTestCodeDuplication, Project project) {
        ...
        if (!classesWithTestCodeDuplication.isEmpty()) {
            //Parte relativa all'inizializzazione del Panel per GeneralFixture.
            this.classesWithTestCodeDuplication = classesWithTestCodeDuplication;
            // Mi prendo tutti i nomi delle classi affette dallo smell.
            for (TestCodeDuplicationInfo testCodeDuplicationInfo : classesWithTestCodeDuplication) {
                //classesNames.add(eti.getClassWithEagerTest().getName());
                if (testCodeDuplicationInfo.getMethodsThatCauseTestCodeDuplication() != null)
                    model.addElement(testCodeDuplicationInfo);
            }
            ...
            if (classesWithTestCodeDuplication.get(0).getMethodsThatCauseTestCodeDuplication() != null)
                secondSplitPane = new TCDSmellPanel(classesWithTestCodeDuplication.get(0), project, this);
    ...
    protected void updateSmellPanel(int index) {
        TestCodeDuplicationInfo testCodeDuplicationInfo;
        if (index == -1 && model.getSize() == 0) {
            return;
        } else if (index == -1 && model.getSize() != 0) {
            testCodeDuplicationInfo = (TestCodeDuplicationInfo) classList.getModel().getElementAt(0);
        } else {
            testCodeDuplicationInfo = (TestCodeDuplicationInfo) classList.getModel().getElementAt(index);
        }
        secondSplitPane = new TCDSmellPanel(testCodeDuplicationInfo, project, this);
    ...
    public void doAfterRefactor() {
        ...
        classesWithTestCodeDuplication.remove(index);
        model.remove(index);
        if (model.getSize() == 0) {
